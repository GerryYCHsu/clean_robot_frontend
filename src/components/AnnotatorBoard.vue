<template>
  <div class="w-full h-full flex flex-col items-center">
    <div
      class="w-full h-full relative focus:outline-none"
      tabindex="1"
      ref="annotatorBox"
    ...
    >
      <div class="flex absolute z-10" :style="annotator.innerToolBox.boxStyle">
        <select
          :value="currentSelectMarkboxLabel"
      ...
      >
        ...
        >
          ...
          >
            <v-rect
              :config="{
                ...
                draggable: true,
                fill: item.labelName === 'cleanZone' ? 'green' : 'red',
                opacity: 0.4
              }"
          ...
          <v-group v-for="item in items" :key="item.image.src">
            <v-image :config="item"/>
          </v-group>
          <v-group v-for="line in lines" :key="line.points">
            <v-line :config="line"/>
          </v-group>
...
const currentEditable = computed(() => {
...
watch(currentEditable, async () => {
  annotator.layerConfig.listening = currentEditable.value;
  if(annotator.layerConfig.listening == false){
    annotator.transformer.detach();
  }
});
...
async function testDrawLine() {
    ...
    if(currentEditable.value == true){
      const fileLines = props.file.lines;
      fileLines[0].points.push(getRandomInt(400))
      fileLines[0].points.push(getRandomInt(400))
      fileLines.forEach(item => lines.push(item));
    }
}
...
async function init(): Promise<void> {
  ...
  const fileItems = props.file.items;
  const fileLines = props.file.lines;
  if (!annotatorBox.value) return;
  await initAnnotator(annotatorBox.value, url, fileMarkboxes);
  markboxes.splice(0);
  if(currentEditable.value==null){
    annotator.layerConfig.listening = true;
  }else{
    annotator.layerConfig.listening = currentEditable.value
  }

  if (fileMarkboxes.length === 0) return;
  fileMarkboxes.forEach((item) => markboxes.push(MarkboxesScale(item)));

  if(fileItems.length === 0) return;
  fileItems.forEach(item => items.push(itemScale(item)));

  if(fileLines.length === 0) return;
  fileLines.forEach(item => lines.push(lineScale(item)));

}

function MarkboxesScale(markbox: Markbox){
  let convertedMarkbox = JSON.parse(JSON.stringify(markbox))
  let scaleRatio = {
    x: annotator.imageConfig.scaleX,
    y: annotator.imageConfig.scaleY,
  }
  convertedMarkbox.width = markbox.width * scaleRatio.x;
  convertedMarkbox.height = markbox.height * scaleRatio.y;
  convertedMarkbox.x = markbox.x * scaleRatio.x;
  convertedMarkbox.y = markbox.y * scaleRatio.y

  return convertedMarkbox
}

function itemScale(item: ItemIcon){
  let convertedItem = {...item}
  let scaleRatio = {
    x: annotator.imageConfig.scaleX,
    y: annotator.imageConfig.scaleY,
  }
  convertedItem.width = item.width * scaleRatio.x;
  convertedItem.height = item.height * scaleRatio.y;
  convertedItem.x = item.x * scaleRatio.x;
  convertedItem.y = item.y * scaleRatio.y;
  
  return convertedItem
}

function lineScale(line: Line){
  let convertedLine = {...line}
  let scaleRatio = {
    x: annotator.imageConfig.scaleX,
    y: annotator.imageConfig.scaleY,
  }
  for(let index=0; index<convertedLine.points.length; index++){
    if(index %2 == 0){
      convertedLine.points[index] = convertedLine.points[index] * scaleRatio.y;
    }else{
      convertedLine.points[index] = convertedLine.points[index] * scaleRatio.x;
    }
  }
...
function startResizeMarkbox(event: KonvaPointerEvent): void {
  ...
  let labels = JSON.parse(JSON.stringify(props.labels))
  if(annotator.layerConfig.listening == true){
    let currentLabel = labels.filter(item => item.color == target.attrs.stroke)
    if(currentLabel.length>0){
      currentSelectMarkboxLabel.value = currentLabel[0].name
    }
  }
